  ...
  export class MapElement extends LitElement {
    ...
    updated(changedProps) {
      ...
      if(changedProps.has("routeData"))
      {
        ...
        {
          if(changedProps.get("routeData").size == 0)
          {
            this.map.invalidateSize();
          }
          ...
          this.polyLines = [];
          this.routeData.forEach((valueMap, valueKey) => {
            ...
            valueMap.forEach((value, key) => {
              ...
              if(prevValue){
                ...
                xhr.open('GET', `https://routing.openstreetmap.de/routed-foot/route/v1/driving/${coordinates[0][1]},${coordinates[0][0]};${coordinates[1][1]},${coordinates[1][0]}?overview=false&geometries=geojson&steps=true`, false);
                xhr.send();
                var status = xhr.status;
                if (status === 200) {
                  var routesJSON = JSON.parse(xhr.response);
